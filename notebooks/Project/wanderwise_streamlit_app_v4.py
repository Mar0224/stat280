...
def normalize_text(value):
    """Normalize categorical labels, especially transport labels, for clean dropdowns and scoring."""
    if pd.isna(value):
        return "Unknown"
    ...
    if text in transport_map:
        return transport_map[text]
...
def get_transport_icon(transport):
    ...
    if any(word in transport for word in ["air", "plane", "airplane", "flight", "fly"]):
        return "✈️"
    if any(word in transport for word in ["train", "rail", "subway", "metro"]):
        return "🚆"
    if any(word in transport for word in ["bus", "coach"]):
        return "🚌"
